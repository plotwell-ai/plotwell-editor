...
router.post('/import-from-script/:projectId', requireAuth, async (req, res) => {
  try {
    const { projectId } = req.params;
    const { clearExisting = false, script_id } = req.body;
    ...
    if (scenes.length === 0) {
      return res.status(400).json({
        ...
        message: 'The script may be empty or not properly formatted with scene headings.'
      });
    }

    // Import scenes to production (handles clearExisting internally)
    const result = await ScriptParsingService.importScenestoProduction(
      projectId,
      userId,
      scenes,
      clearExisting
    ...
    const importedCount = result.sceneCards.length;
    const skippedCount = scenes.length - importedCount;

    let message = '';
    if (clearExisting) {
      message = `Imported ${importedCount} scenes from "${scriptTitle}"`;
    } else if (skippedCount > 0) {
      message = `Imported ${importedCount} new scenes from "${scriptTitle}". Skipped ${skippedCount} existing scenes.`;
    } else {
      message = `Imported ${importedCount} scenes from "${scriptTitle}"`;
    }
    ...
    res.json({
      ...
      scenesCount: importedCount,
      skippedCount,
      totalScenesInScript: scenes.length,
      scriptTitle
    });
...
// Import scenes from script to storyboard
router.post('/import-to-storyboard/:projectId', requireAuth, async (req, res) => {
  try {
    const { projectId } = req.params;
    ...
    // Parse script and extract scenes (uses prod_script_id by default)
    const { storyboardScenes, scriptTitle, scriptId } = await ScriptParsingService.parseScriptFromProject(
      projectId,
      userId,
      script_id
    );
    ...
    const storyboard = await ScriptParsingService.importScenesToStoryboard(
      projectId,
      userId,
      storyboardScenes
    );
    ...
    res.json({
      success: true,
      message: `Created storyboard with ${storyboardScenes.length} scenes from "${scriptTitle}"`,
      storyboard,
    ...
    res.status(500).json({
      error: 'Failed to import to storyboard',
      details: error instanceof Error ? error.message : 'Unknown error'
    });
  }
});
 ...
 */
router.post('/resolve-changes/:projectId', requireAuth, async (req, res) => {
  try {
    const { projectId } = req.params;
    const userId = getUserId(req);
    ...
    if (!Array.isArray(approvedScenes) || !Array.isArray(rejectedScenes) || !Array.isArray(deletedScenes)) {
      return res.status(400).json({ error: 'Invalid request body' });
    }

    if (!strategy || !['replace', 'merge'].includes(strategy)) {
      return res.status(400).json({ error: 'Invalid strategy. Must be "replace" or "merge"' });
    }

    const result = await resolveChanges(projectId, userId, {
      approvedScenes,
      rejectedScenes,
      deletedScenes,
      strategy
    ...
    res.json({
      ...result,
      message: `Updated ${result.updated} scene(s), deleted ${result.deleted} scene(s).`
    });

  } catch (error) {
    console.error('Error resolving changes:', error);
    res.status(500).json({
      error: 'Failed to resolve changes',
      details: error instanceof Error ? error.message : 'Unknown error'
    });
  }
});
...
/**
 * POST /api/production/scenes/:sceneId/unlock
 * Unlock a scene to allow sync
 */
router.post('/scenes/:sceneId/unlock', requireAuth, async (req, res) => {
  try {
    const { sceneId } = req.params;
    const userId = getUserId(req);
